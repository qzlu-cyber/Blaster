...
/// 2. 在 server 端，且武器的 bUsedServerSideRewind 为 true，但不是本地控制的那么开枪的就是其他 client，生成 non replicated 且 bUsedServerSideRewind 为 true 的 projectile
/// 这是因为此 client 将生成自己的本地 non replicated 的 projectile，之后再向 server 端发送 ServerScoreRequest
			...
			if (bUseServerSideRewind) // 如果武器需要使用服务器端回滚
			{
				...
				else
				{
					if (InstigatorPawn->IsLocallyControlled()) // 本地控制的 Character
					{
						...
						SpawnedProjectile->LaunchVelocity = SpawnedProjectile->GetActorForwardVector() * SpawnedProjectile->GetProjectileMovementComponent()->InitialSpeed;
